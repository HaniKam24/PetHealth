...
Per `docs/PRD.md`'s Bolt numbering: Phase 1 (Bolts 1–10, the MVP) plus Bolt 12 (trend insights, built ahead of schedule) are done. Bolts 19-21 (Pawlie: the AI assistant persona, full-context grounding, and confirmed conversational actions) are merged. Bolts 22-24 (Symptom Journal + rule-based predictive alerts — see the gotcha below) are built and tested locally, open for review as PRs #44/#46/#47. Bolt 25 (letting an owner attach a note when they resolve/dismiss an alert) is not started. Bolt 11 (daily digest + recurring reminders) turned out to have no actual code behind its earlier "code-complete" status (verified directly against the repo) — it's deprioritized behind the Predictive Health Monitoring work and not currently being worked on. Bolt 13 (co-owner invites) is Phase 3, explicitly deferred. Phase 3 more broadly (14–18) is also deferred in the PRD.
...
- Predictive Health Monitoring (`artifacts/api-server/src/lib/predictive-monitoring.ts`, `routes/alerts.ts`, `lib/db/src/schema/alerts.ts`): watches a pet's Symptom Journal entries (`symptom_entries`, Bolt 22) and flags a pattern worth a second look — low energy/appetite, diarrhea, vomiting, limping — each threshold cited from real veterinary guidance (`docs/PRD.md` §7e), not learned from this app's own data. It re-runs synchronously right after a journal entry is created or deleted, same trigger model as the Care Recommendations Engine above. **It will never try to connect a symptom to a specific medication or diagnosis** — that's a deliberate, hard line, not just caution in the prompt: the `reasoning` it stores literally can't carry that kind of claim (`historicalDataPoint` is always `null` in this version), and every flagged alert instead offers an "Ask Pawlie about this" handoff so a real judgment call goes to Pawlie, not a hardcoded rule. One active alert per pet at a time: a new pattern updates the existing active alert in place, and it auto-resolves itself once nothing is flagged anymore (no cron job needed — it's checked on the fly, same as everything else here). An owner can also dismiss an alert manually from the dashboard banner.

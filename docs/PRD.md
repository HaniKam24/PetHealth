...
**Status:** Draft v1.6 (MVP scope confirmed via stakeholder Q&A)
**Prepared:** 2026-09-06
**Stage:** Phase 1 done; Phase 2 in progress (Bolt 12 shipped; Bolts 19–21 scoped; Bolts 22–25 newly scoped and prioritized ahead of Bolt 11, whose prior "code-complete" status didn't check out — see §11)
...
**`alerts`** *(new, Bolt 23/24)* — `id` serial pk · `pet_id` → pets.id · `triggered_by_entry_id` → symptom_entries.id · `severity` text: green\|yellow\|red · `confidence` text: low\|medium\|high *(derived from how many of the last N entries support the flagged pattern — see §7e)* · `reasoning` jsonb *(structured list of `{signal, value, historicalDataPoint, weight}` — the source data a template renders into the shown explanation, never freeform AI text)* · `status` text: active\|dismissed\|resolved · `outcome_note` text, nullable *(Bolt 25 — set when an owner resolves/dismisses with an optional note)* · `created_at` / `resolved_at` timestamptz.
...
- **Confidence weighting is entry-count-based, not calendar-day-based.** Nothing in the app today prompts an owner to log daily, so a literal "three consecutive days" check would rarely fire on real, gappy data. Instead: a pattern is flagged when it holds across most of a pet's last few entries within a lookback window (e.g. "low energy in 3 of the last 4 entries, within 7 days") — tolerant of gaps, and the same ratio doubles as the alert's confidence level.

**The cold-start problem — sequencing, not just an implementation detail:** there is zero structured symptom data in this system today; the only existing symptom-related table (`symptom_logs`) is a handful of free-text notes an owner chose to save from a one-off chat answer. A trend-detection engine (Bolt 23) has nothing real to validate against until the journal (Bolt 22) has shipped and actually been used for a while. Bolt 23 is marked blocked-on-usage in the roadmap below for exactly this reason — a real, code-verified block, not an aspirational one.
...
- Bolt 22 — Predictive monitoring: symptom journal — `symptom_entries` table (appetite/energy/stool/vomiting/limping/behavior, all nullable, weight routed to the existing `weight_logs`), a dedicated logging UI (not just the existing chat-derived "add to symptom log"). Ship this and use it for real before starting Bolt 23 — see the cold-start note in §7e.
- Bolt 23 — Predictive monitoring: baseline & trend detection — **blocked on Bolt 22 shipping and accumulating real entries.** Rule-based, entry-count-windowed comparison against that pet's own recent entries (e.g. "low energy in 3 of the last 4 entries, within 7 days"), confidence derived from the same ratio. No AI-provider calls. See §7e.
- Bolt 24 — Predictive monitoring: severity tiers & alerts — `alerts` table; combines Bolt 23's trend signal with existing health records/active medications/lab results into green/yellow/red; yellow surfaces as a dashboard card, red adds a `tel:`-link-to-saved-vet-contact action plus a copyable summary (extends the existing symptom-chat escalation pattern, §7c); every alert renders its `reasoning` jsonb through a fixed template — no LLM-generated explanation text. See §7e.
- Bolt 25 — Predictive monitoring: outcome logging — an owner can mark an `alerts` row resolved/dismissed with an optional one-field outcome note (`outcome_note`, `resolved_at`); deliberately lightweight for MVP, sets up (but doesn't require) swapping in a trained model later per the original issue's "leave room for ML" requirement.
...
- **Risk:** Bolt 23's trend detection is built and "validated" against synthetic or team-only test data, then behaves oddly against real owners' sparse, irregular logging habits. *Mitigation:* Bolt 23 is explicitly sequenced behind Bolt 22 shipping and accumulating real usage (§7e) — not started until there's real data to tune the entry-count window against.
...
- **Open question:** exact entry-count/lookback-window thresholds for Bolt 23 (e.g. "3 of the last 4 entries, 7-day window") are a starting proposal, not tested against real data — expect to tune once Bolt 22 has real usage.

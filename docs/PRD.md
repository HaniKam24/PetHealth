...
**Status:** Draft v1.7 (MVP scope confirmed via stakeholder Q&A)
**Prepared:** 2026-09-06
**Stage:** Phase 1 done; Phase 2 in progress (Bolts 12, 19, 20, 21, 22 shipped; Bolt 23–25 thresholds now research-grounded and ready to build; Bolt 11 deprioritized — see §11)
...
**The cold-start problem — sequencing, not just an implementation detail:** there is zero structured symptom data in this system today; the only existing symptom-related table (`symptom_logs`) is a handful of free-text notes an owner chose to save from a one-off chat answer. A trend-detection engine (Bolt 23) has nothing real to validate against until the journal (Bolt 22) has shipped and actually been used for a while. Bolt 23 is marked blocked-on-usage in the roadmap below for exactly this reason — a real, code-verified block, not an aspirational one. The thresholds above are the starting point, not assumed-final — worth revisiting once real usage data exists to check them against, same as the unit-cost estimates elsewhere in this doc.

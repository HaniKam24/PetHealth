...
- medications: one entry per medication/prescription mentioned, "dose" as written (e.g. "5mg", "1 tablet"). The dose is often written combined with the medication's name rather than as its own field (e.g. "Amoxicillin 250mg", "Rimadyl (carprofen) 75 mg tablets", "Doxycycline Hyclate 100mg Tablet") — in that case, split it: put the numeric/unit portion in "dose" and keep only the medication's name in "name" (e.g. name: "Amoxicillin", dose: "250mg"). Only set doseIntervalValue/doseIntervalUnit if the document states a clear fixed interval (e.g. "twice daily" -> 12/"hours", "every 30 days" -> 30/"days"); otherwise leave both null and describe the schedule in "frequency" as free text.
